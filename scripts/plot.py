...
fig, ax = plt.subplots(figsize=(10, 6))
for pc, g in agg.groupby("particle_count"):
...
ax.set_xscale("log", base=2)
wg_ticks = sorted(agg["workgroup_size"].unique())
ax.set_xticks(wg_ticks)
ax.set_xticklabels([str(v) for v in wg_ticks])
ax.set_xlabel("Workgroup size")
ax.set_ylabel("Dispatch time (µs)")
ax.set_title("GPU dispatch latency vs workgroup size")
...
PANEL_PARTICLES = [65536, 262144, 1048576, 2097152]
PANEL_LABELS    = ["65k — fits in L2", "262k — fits in Infinity Cache", "1M — near the edge", "2M — spilling to GDDR6"]
WG_FOCUS = 256
...
axes[1, 1].sharey(axes[1, 0])  # bottom row shares its own scale
